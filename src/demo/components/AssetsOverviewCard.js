...
class AssetsOverviewCard extends Component
{
	...
	refresh = async () =>
	{
		let accounts = await this.props.services.web3.eth.getAccounts()
		let balance  = await this.props.services.web3.eth.getBalance(accounts[0])
	...
	render()
	{
		return (
			<MDBCard>
				<MDBCardHeader>
					Account balance for { this.state.account }
				</MDBCardHeader>
				<MDBCardBody>
